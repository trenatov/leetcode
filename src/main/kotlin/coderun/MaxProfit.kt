...
fun main(args: Array<String>) {
    ...
    val sellers = reader.readLine().split(" ").map { it.toInt() }.sorted()
    val buyers = reader.readLine().split(" ").map { it.toInt() }.sorted()

    var j = buyers.lastIndex
    val min = Math.min(sellers.size, buyers.size)
    var maxProfit = 0

    for (i in 0 until min) {
        if (sellers[i] > buyers[j]) break

        maxProfit += buyers[j] - sellers[i]
        j--
    }
